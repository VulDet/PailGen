...
stmt_types = ['continue_statement', 'labeled_statement', 'expression_statement', 'goto_statement', 'if_statement',
             'local_function_statement', 'return_statement', 'local_declaration_statement', 'do_statement', 'switch_statement',
             'break_statement', 'global_statement', 'for_statement', 'while_statement', 'method_declaration',
              'block', 'struct_declaration', 'field_declaration', 'constructor_declaration',
              ]

class ChangeNode(object):
    def __init__(self, node, lineno, end_lineno, change_lines, raw_change_lines, text, ctx = None):
        self.node = node
        self.lineno = lineno
    ...
    def set_type(self):
        if self.node.type in stmt_types:
            self.type = 'stmt'
        else:
            self.type = 'expr'
    ...
    def gen_children_as_ast(self):
        for n in self.node.children:
            ...
            c_node = ChangeNode(n, lineno, end_lineno, [], [], None)
...
class ChangeTree(object):
    ...
    def get_loc(node):
        ...
        while(len(nodes) != 0):
            n = nodes[0]
            nodes = nodes[1:]
            if hasattr(n, 'start_point') and n.start_point[0]+1 < min_lineno:
                min_lineno = n.start_point[0]+1
            if hasattr(n, 'end_point') and n.end_point[0]+1 > max_lineno:
                max_lineno = n.end_point[0]+1
            for c in n.children:
                nodes.append(c)
        ...
        for n in change_nodes:
            ...
            for cn in ast_parent.children:
                ...
                if lineno > max_line and lineno not in n.parent.raw_change_lines:
                    c_node = ChangeNode(cn, lineno, end_lineno, [], [], None)
                    if c_node.type == 'stmt':
                        c_node.gen_children_as_ast()
            ...
            for n in node.node.children:
                ...
                for l in self.raw_change_lines:
                    if l in range(lineno, end_lineno + 1):
                        raw_lines.append(l)
                if len(lines) != 0:
                    c_node = ChangeNode(n, lineno, end_lineno, lines, raw_lines, n.text)
